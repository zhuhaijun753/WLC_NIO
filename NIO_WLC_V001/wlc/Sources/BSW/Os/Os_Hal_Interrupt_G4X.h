...
#ifndef OS_HAL_INTERRUPT_G4X_H
# define OS_HAL_INTERRUPT_G4X_H
...
# define OS_HAL_LEVEL_MASK(Level)      (uint32)Level                                                                    /* PRQA S 3453 */ /* MD_MSR_FctLikeMacro */
 ...
 *            and G4X cores.
 *            On G4X cores PLMR register is used to mask certain interrupt
 *            priority levels. 5 Bits are used to specify priority to be
 *            masked. Writing 0 (highest priority level) to PLMR register
 *            masks all interrupt priorities. Writing 4 to PLMR register
 *            means interrupt must have priority of 3 or higher to be serviced.
 */
...
{
  *Level = Os_Hal_GetLevelG4X();                                                                                           /* SBSW_OS_HAL_PWA_CALLER */
}
...
))
{
  ...
  InterruptState->PMR = Os_Hal_GetLevelG4X();                                                                           /* SBSW_OS_HAL_PWA_CALLER */
}
...
))
{
  ...
  if(OS_HAL_EIBD2_BASE != 0u)                                                                                           /* PRQA S 2741 */ /* MD_Os_Hal_Rule14.3_2741 */
  {
    ...
    IsrConfig->IBDnRef->IBDnReg = (uint32)(IsrConfig->CoreID);                                                          /* SBSW_OS_HAL_PWA_PA_ISRCONFIG */
  }
}
...
))
{
  ...
  IsrConfig->EICnRef->EICnHi |= OS_HAL_INTC_INT_SET_MASK;                                                               /* SBSW_OS_HAL_PWA_PA_ISRCONFIG */
...
))
{
  ...
  IsrConfig->EICnRef->EICnHi &= OS_HAL_INTC_INT_CLR_MASK;                                                               /* SBSW_OS_HAL_PWA_PA_ISRCONFIG */
...
{
  return ((IsrConfig->EICnRef->EICnHi & OS_HAL_INTC_INT_SET_MASK) == 0x0u);                                             /* PRQA S 4404 */ /* MD_Os_C90BooleanCompatibility */
}
...
))
{
  ...
  IsrConfig->EICnRef->EICnHi = (uint16)(IsrConfig->EICnRef->EICnHi & 0x0080u)
                                    | OS_HAL_INTC_TABLE_MODE;
  IsrConfig->EICnRef->EICnLo = ((uint16)(IsrConfig->IntLevel));
}
...
))                                                                                                                      /* COV_OS_HALPLATFORMPITUNSUPPORTED_XX */
{
  ...
  return (uint8)((IsrConfig->EICnRef->EICnHi & OS_HAL_INTC_REQUEST_FLAG)>>8);
}
...
/***********************************************************************************************************************
 *  END OF FILE: Os_Hal_Interrupt_G4X.h
 **********************************************************************************************************************/
